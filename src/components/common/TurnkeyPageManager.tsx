              ...
              <div key={btn.id} className="p-4 border rounded bg-white relative space-y-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    <label className={labelClass}>Link <input className={fieldClass} placeholder="https://" value={btn.link || ''} onChange={e => { const nb = [...data.buttons]; nb[idx] = { ...nb[idx], link: e.target.value }; setForm({ ...form, data: { ...data, buttons: nb } }) }} /></label>
                  </div>
