          ...
          <form onSubmit={handleSave} className="bg-white border border-[#ded3c4] rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="pt-4">
                ...
                {/* Hero section with background image, title, and actions */}
                {form.key === "about.hero" && renderHeroForm()}
                {/* Detailed story section with stats and core values */}
                {form.key === "about.ourStory" && renderOurStoryForm()}
                {/* Horizontal strip with icons and quick stats */}
                {form.key === "about.statsStrip" && renderStatsStripForm()}
                {/* Section highlighting unique selling points and experience */}
                {form.key === "about.whyChooseEnsis" && renderWhyChooseEnsisForm()}
                {/* Grid of expertise items with thumbnails and descriptions */}
                {form.key === "about.ourExpertise" && renderExpertiseForm()}
                {/* Visual process diagram and step-by-step description */}
                {form.key === "about.ourTurnkeyProcess" && renderTurnkeyProcessForm()}
                {/* Grid layout of industries served with links */}
                ...
                {/* Founder's personal message and signature section */}
                {form.key === "about.founderVision" && renderFounderVisionForm()}
                {/* Final Call to Action section for starting projects */}
                {form.key === "about.letsBuild" && renderLetsBuildForm()}
                {/* Site-wide header configuration (logo, navigation, contacts) */}
                {form.key === "layout.header" && <HeaderForm data={form.data as HeaderData} setData={setData} uploadingField={uploadingField} setUploadingField={setUploadingField} />}
                {/* Site-wide footer configuration (company info, social, links) */}
                {form.key === "layout.footer" && <FooterForm data={form.data as FooterData} setData={setData} uploadingField={uploadingField} setUploadingField={setUploadingField} />}
