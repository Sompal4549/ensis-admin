  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-100 bg-white shadow-sm">
        ...
        {links.length === 0 ? (
          <p className="py-10 text-center text-xs text-slate-400">No links yet. Add one →</p>
        ) : (
              ...
              <div key={link._id} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50/50 transition-colors">
                <div className="h-9 w-9 shrink-0 rounded-full border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center">
                  {link.icon
                    ...
                    : <span className="text-[11px] font-black text-slate-300 uppercase">{link.platform?.[0]}</span>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className="text-[9px] text-slate-400">#{link.order}</span>
                  </div>
                  <p className="mt-0.5 text-[11px] text-slate-500 truncate">{link.url}</p>
                </div>
    ...
    >
      <X size={13} />
        ...
        <div className="flex gap-2 pt-1">
          ...
          {editingId && (
            <button onClick={reset} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-[11px] font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
              <X size={11} /> Cancel
            </button>
          )}
        </div>
      ...
      <div className="flex items-center gap-1 border-b border-slate-100">
        {([["links", Link2, "Links Manager"], ["analytics", BarChart2, "Click Analytics"]] as const).map(([id, Icon, label]) => (
          <button key={id} onClick={() => setTab(id)}
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold border-b-2 -mb-px transition-colors ${tab === id ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
            <Icon size={13} /> {label}
          </button>
        ))}
      </div>
      ...
      {tab === "analytics" && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            ...
            {statsList.map(s => (
              <button key={s._id} onClick={() => { setFilterPlatform(filterPlatform === s._id ? "" : s._id); setPage(1); }}
                ...
                <p className={`text-[9px] font-bold capitalize ${PLATFORM_COLORS[s._id]?.split(" ")[1] ?? "text-slate-600"}`}>{s._id}</p>
          ...
          <div className="space-y-2">
            {loading ? (
              <div className="rounded-xl border bg-white p-10 text-center text-xs text-slate-400">Loading...</div>
            ) : Object.keys(groupedClicks).length === 0 ? (
              <div className="rounded-xl border bg-white p-10 text-center text-xs text-slate-400">No clicks recorded yet.</div>
              ...
              <details key={platform} className="overflow-hidden rounded-lg border bg-white">
                <summary className="flex cursor-pointer items-center justify-between px-3 py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-slate-500">{items.length} clicks</span>
                  </div>
