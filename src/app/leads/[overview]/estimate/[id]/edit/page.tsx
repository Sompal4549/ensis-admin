...
import { invoiceApi, leadApi, Lead, Product, productApi, Category, categoryApi, Invoice } from "@/lib/api";
import { toast } from "react-toastify";
import { fieldClass, labelClass } from "@/constants";

export default function EditEstimatePage({ params }: { params: Promise<{ overview: string; id: string }> }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [invoiceRes, leadRes, productsRes, categoriesRes] = await Promise.all([
        ...
        setBillingName(inv.billingAddress?.name || "");
        setBillingContactPerson(inv.billingAddress?.name || "");
        setBillingMobile(inv.billingAddress?.phone || "");
        setBillingEmail(inv.billingAddress?.email || "");
        setBillingAddress(inv.billingAddress?.addressLine || "");
        setBillingCity(inv.billingAddress?.city || "");
        setBillingState(inv.billingAddress?.state || "");
        setBillingCountry(inv.billingAddress?.country || "India");
        setBillingPinCode(inv.billingAddress?.postalCode || "");
        setBillingGst(inv.billingAddress?.gstNumber || "");

        setConsigneeName(inv.shippingAddress?.name || "");
        setConsigneeContactPerson(inv.shippingAddress?.name || "");
        setConsigneeMobile(inv.shippingAddress?.phone || "");
        setConsigneeEmail(inv.shippingAddress?.email || "");
        setConsigneeAddress(inv.shippingAddress?.addressLine || "");
        setConsigneeCity(inv.shippingAddress?.city || "");
        setConsigneeState(inv.shippingAddress?.state || "");
        setConsigneeCountry(inv.shippingAddress?.country || "India");
        setConsigneePinCode(inv.shippingAddress?.postalCode || "");
        setConsigneeGst(inv.shippingAddress?.gstNumber || "");
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-slate-800">Edit Estimate</h1>
        </div>
      </div>
