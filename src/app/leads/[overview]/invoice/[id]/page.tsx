...
import { ArrowLeft, Loader2, Pencil, Printer, Landmark, PenLine, FileText } from "lucide-react";
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .header-left { display:flex; align-items:center; gap:12px; padding:20px 24px; border-right:1px solid rgba(255,255,255,0.2); }
  .header-left img { width:56px; height:56px; background:#fff; border-radius:8px; padding:4px; object-fit:contain; flex-shrink:0; }
  .header-left p { font-size:11px; line-height:1.6; }
  .header-mid { padding:20px 24px; border-right:1px solid rgba(255,255,255,0.2); display:flex; flex-direction:column; justify-content:center; }
  .header-mid p { font-size:11px; line-height:1.6; }
  .header-right { padding:20px 24px; text-align:right; font-size:11px; line-height:1.6; display:flex; flex-direction:column; justify-content:center; }
  .title-bar { text-align:center; padding:16px; border-bottom:2px solid #1a3a5c; }
  .title-bar h2 { font-size:16px; letter-spacing:3px; text-transform:uppercase; color:#1a3a5c; }
  ...
  .info-label { background:#1a3a5c; color:#fff; font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:1px; padding:4px 8px; margin:-16px -16px 8px -16px; }
  .info-col p { font-size:11px; line-height:1.7; }
  table { width:100%; border-collapse:collapse; font-size:11px; }
  th { background:#1a3a5c; color:#fff; padding:8px; text-align:left; font-size:10px; }
  ...
  .totals .grand { border-top:2px solid #1a3a5c; padding-top:8px; margin-top:4px; font-size:14px; font-weight:700; }
  .terms-grid { display:grid; grid-template-columns:1fr 1fr; gap:16px; padding:16px; border-top:1px solid #e5e7eb; }
  .terms-box { border:1px solid #e5e7eb; border-radius:6px; padding:12px; }
  .terms-box h4 { font-size:11px; font-weight:700; margin-bottom:8px; }
  .terms-box ol { padding-left:16px; font-size:10px; line-height:1.8; }
  .bottom-grid { display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px; padding:0 16px 16px 16px; }
  .bottom-box { border:1px solid #e5e7eb; border-radius:6px; padding:12px; font-size:11px; }
  .bottom-box h4 { font-weight:700; margin-bottom:8px; }
  .footer { background:#1a3a5c; color:#fff; text-align:center; padding:12px; font-size:10px; }
</style></head><body>
<div class="container">
  <div class="header">
  ...
  <div class="title-bar"><h2>Tax Invoice</h2></div>
  ...
  <div class="info-grid">
    ...
    <div class="info-col">
      <div class="info-label" style="text-align:right">Invoice Details</div>
      <div style="display:flex;flex-direction:column;gap:4px">
        <div style="display:flex;justify-content:space-between"><span>Invoice No. :</span><strong>${inv.invoiceNumber}</strong></div>
        <div style="display:flex;justify-content:space-between"><span>Invoice Date :</span><span>${invDate}</span></div>
        ${inv.dueDate ? `<div style="display:flex;justify-content:space-between"><span>Due Date :</span><span>${new Date(inv.dueDate).toLocaleDateString("en-IN")}</span></div>` : ""}
        <div style="display:flex;justify-content:space-between"><span>Created Date :</span><span>${invDate}</span></div>
        <div style="display:flex;justify-content:space-between"><span>Created Time :</span><span>${invTime}</span></div>
  ...
  <div style="padding:16px">
    <table>
      ...
      <tbody>${itemRows || '<tr><td colspan="10" style="text-align:center;padding:16px;color:#9ca3af">No items</td></tr>'}</tbody>
    </table>
  </div>
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="max-w-[900px] mx-auto bg-white shadow-lg rounded-lg overflow-hidden print:shadow-none print:rounded-none print:max-w-full">
        ...
        <div className="text-center py-4 border-b-2 border-[#1a3a5c]">
          <h2 className="text-lg font-bold tracking-widest text-[#1a3a5c] uppercase">Tax Invoice</h2>
        </div>
        ...
        <div className="grid grid-cols-3 border-b text-[11px]">
          ...
          {/* Invoice Details */}
          <div className="p-4">
            <p className="font-bold text-[10px] uppercase tracking-wider bg-[#1a3a5c] text-white px-2 py-1 -mx-4 -mt-4 mb-2 text-right">Invoice Details</p>
            <div className="space-y-1">
              <div className="flex justify-between"><span>Invoice No. :</span><span className="font-semibold">{invoice.invoiceNumber}</span></div>
              <div className="flex justify-between"><span>Invoice Date :</span><span>{new Date(invoice.createdAt).toLocaleDateString("en-IN")}</span></div>
              {invoice.dueDate && <div className="flex justify-between"><span>Due Date :</span><span>{new Date(invoice.dueDate).toLocaleDateString("en-IN")}</span></div>}
        ...
        <div className="grid grid-cols-3 gap-4 px-4 pb-4 text-[11px]">
          <div className="border rounded-lg p-3">
            <p className="font-bold mb-2 flex items-center gap-1">
              <Landmark size={14} /> {COMPANY.shortName} BANK DETAILS
            </p>
            <div className="space-y-1">
              <p>Bank Name : {COMPANY.bank.name || "--"}</p>
              <p>Account Name : {COMPANY.bank.accountName || "--"}</p>
          ...
          <div className="border rounded-lg p-3">
            <p className="font-bold mb-2 flex items-center gap-1">
              <PenLine size={14} /> RECEIVER&apos;S ACKNOWLEDGEMENT
            </p>
          ...
          <div className="border rounded-lg p-3">
            <p className="font-bold mb-2 flex items-center gap-1">
              <FileText size={14} /> FOR {COMPANY.brandName}
            </p>
            <div className="mt-8 border-t border-dashed pt-2 text-center text-[10px] text-slate-400">
