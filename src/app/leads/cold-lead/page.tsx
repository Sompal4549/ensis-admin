...
export default function ColdLeadPage() {
  ...
  const fetchLeads = async (p = page) => {
    setLoading(true);
    try {
      const res = await leadApi.list({ leadCategory: "cold", search: search || undefined, page: p, limit: PAGE_SIZE });
      setLeads(res.leads);
      setTotal(res.total);
    } catch {
  ...
  const handleMarkHot = async (lead: Lead) => {
    try {
      await leadApi.update(lead._id, { leadCategory: "hot" });
      toast.success("Moved to hot");
      fetchLeads();
    } catch {
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-base font-bold text-slate-800">Cold Leads</h1>
        <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">{total}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className={`${cardClass} text-center py-4`}>
          <p className="text-xs text-slate-500">No cold leads found</p>
        </div>
      ) : (
        <>
              ...
              <tbody>
                {leads.map((lead) => (
                  <tr key={lead._id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer" onClick={() => router.push(`/leads/${lead._id}`)}>
                    <td className="px-3 py-1">
                      <div className="flex items-center gap-2">
                        {lead.leadImage ? (
                          ...
                          <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-semibold text-[9px]">{lead.firstName[0]}{lead.lastName[0]}</div>
                        )}
                        <span className="font-medium text-slate-800 truncate max-w-30">{lead.firstName} {lead.lastName}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2 text-slate-800 hidden md:table-cell truncate max-w-35">{lead.companyName}</td>
                    <td className="px-3 py-2 text-slate-800 hidden lg:table-cell truncate max-w-40">{lead.email}</td>
                    ...
                    <td className="px-3 py-2 text-slate-700 hidden xl:table-cell truncate max-w-25">
                      {lead.assignedTo && typeof lead.assignedTo === "object" ? lead.assignedTo.name : "-"}
                    </td>
                    <td className="px-3 py-2 text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleMarkHot(lead)} className="p-1 rounded hover:bg-orange-50 text-slate-400 hover:text-orange-600 text-[10px]" title="Move to Hot">Hot</button>
